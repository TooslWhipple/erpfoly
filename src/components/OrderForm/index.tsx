...
export default function OrderForm({
  ...
  const orderType =
    propOrderType ??
    (router.query.orderType as "external" | "internal" | undefined) ??
    "external";
  ...
  const [originBranch, setOriginBranch] = useState<BranchCatalogItem | null>(
    null,
  );
  ...
  const [originalOrder, setOriginalOrder] = useState<OrderFullDetail | null>(
    null,
  );
  ...
  const resolvedSupplierName =
    propSupplierName ?? (router.query.supplierName as string | undefined);
  const resolvedBranchId =
    propBranchId ?? (router.query.branchId as string | undefined);
  const resolvedBranchName =
    propBranchName ?? (router.query.branchName as string | undefined);
  const resolvedOriginBranchId =
    propOriginBranchId ?? (router.query.originBranchId as string | undefined);
  const resolvedOriginBranchName =
    propOriginBranchName ??
    (router.query.originBranchName as string | undefined);
  useEffect(() => {
    if (mode === "create") {
      if (
        orderType === "external" &&
  ...
  const loadOrder = async (id: number) => {
    ...
    try {
      ...
      if (orderData) {
        ...
        const items: SelectedOrderItem[] = orderData.order_items.map(
          (item) => ({
  ...
  const { data: productRows, isFetching: articlesFetching } = useQuery({
    queryKey: [
      effectiveOrderType === "external"
  ...
  const handleContinue = async () => {
    ...
    if (mode === "create") {
      ...
      try {
        ...
        if (result.data) {
          const returnUrl =
            originalOrder?.order_type === "internal"
              ? `/traspasos/${orderId}`
              : `/pedidos/${orderId}`;
  ...
  const truncatedName = (name: string | undefined) => {
    ...
    return name.slice(0, 40).split(" ").slice(0, -1).join(" ") + "...";
  };
  const displayName =
    effectiveOrderType === "external"
      ? supplier?.name
  ...
  const breadcrumbs: BreadcrumbItem[] =
    mode === "create"
      ? [
          {
            label: "Traspasos",
            href: effectiveOrderType === "internal" ? "/traspasos" : "/pedidos",
          },
          {
            label: truncatedName(displayName),
            href:
              displayName && branch
                ? effectiveOrderType === "internal"
                  ? `/traspasos?branch=${branch?.id}`
                  : `/pedidos?supplier=${supplier?.id}`
                : undefined,
          },
          {
            label:
      ...
      : [
          ...
          {
            label: originalOrder?.folio
              ? `Pedido ${originalOrder.folio}`
              : "...",
            href: originalOrder
              ? originalOrder.order_type === "internal"
                ? `/traspasos/${originalOrder.id}`
                : `/pedidos/${originalOrder.id}`
              : undefined,
          },
          {
            label: "Editar pedido",
  ...
  const columns: Column<ArticleRow>[] = [
    ...
    },
    {
      ...
      format: (value) => {
        ...
        return <StockCell isLow={isLowStock}>{stock} unidades</StockCell>;
      },
    },
    {
      id: "pendingSupply",
