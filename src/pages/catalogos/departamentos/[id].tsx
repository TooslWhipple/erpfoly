...
import {
  formatPercentFieldValue,
  getProfitMarginFieldState,
...
export default function DepartmentDetailPage() {
  ...
  const fetchDepartment = useCallback(async () => {
    ...
    if (result.error) {
      console.error(
        "[DepartmentDetail] Error fetching department:",
  ...
  const marginFieldState = useMemo(
    () =>
      department
        ? getProfitMarginFieldState(marginDraft, department.margin)
        : null,
  ...
  const handleSaveGroup = useCallback(
    async (data: LineFormOutput) => {
      ...
      if (editingLine) {
        ...
        if (result.error) {
          console.error(
            "[DepartmentDetail] Error saving line:",
            result.error.message,
          );
  ...
  const handleDeleteGroup = useCallback(
    async (row: DepartmentLineTableRow) => {
      ...
      if (result.error) {
        console.error(
          "[DepartmentDetail] Error deleting line:",
          result.error.message,
        );
        showError(result.error.message);
        return;
  ...
  const handleSaveSettings = useCallback(async () => {
    ...
    const result = await updateDepartment(departmentId, {
      margin: marginFieldState.parsed,
    });
    setSavingSettings(false);
    if (result.error) {
      console.error(
        "[DepartmentDetail] Error updating margin:",
        result.error.message,
      );
      showError(result.error.message);
      return;
