...
function XAxisTick({
  ...
  const payloadArr = Array.isArray(payload)
    ? payload
    : payload
      ? [payload]
      : [];
...
export default function MetasPage() {
  ...
  const currentMetricLabel =
    CHART_METRIC_OPTIONS.find((o) => o.value === chartMetric)?.label ??
    "Ventas";
  ...
  return (
    <Stack direction="column" spacing={3}>
      <Title title="Metas" description="Configura las metas por sucursal" />

      <Card>
        <Typography variant="h6">Historial de ventas</Typography>
        <Button variant="outlined" size="small" onClick={handleChartFilterOpen}>
          {currentMetricLabel}
        </Button>
        <Menu
        ...
        <ChartWrapper>
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <CartesianGrid
                strokeDasharray="3 3"
                stroke={theme.palette.app.border}
              />
              <XAxis
                dataKey="label"
                tickLine={false}
        ...
        >
          <Stack spacing={0.5} flex={6}>
            ...
            <Typography variant="body2" color="text.secondary">
              El monto total de la meta mensual para esta sucursal está
              conformado por la meta individual de cada vendedor asignado.
            </Typography>
          </Stack>
          <Typography variant="h4" align="right" flex={1}>
            {numeral(totalGoal).format("$0,0.00")}
          </Typography>
        </Stack>
      </Card>
