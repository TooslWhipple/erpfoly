...
import { getDepartmentDetail, applyPriceSuggestion } from "@/data/liquidaciones.mockData";
import { ArticlesGrid, RulesList } from "@/styles/inventario/departamento.styles";
...
export default function DepartamentoLiquidacionesPage() {
  ...
  const fetchDetail = useCallback(async () => {
    ...
    try {
      ...
      setDepartment(data);
      setRules(data.rules);
      setState(data.articles.length === 0 && data.rules.length === 0 ? "empty" : "success");
    } catch {
      setState("error");
    }
  }, [departmentId]);
  ...
  const handleRuleOperatorChange = useCallback((ruleId: string, value: LiquidationRuleOperator) => {
    setRules((prev) => prev.map((r) => (r.id === ruleId ? { ...r, operator: value } : r)));
  }, []);

  const handleRuleValueChange = useCallback((ruleId: string, value: number) => {
    setRules((prev) => prev.map((r) => (r.id === ruleId ? { ...r, value } : r)));
  }, []);

  const handleRulePeriodChange = useCallback((ruleId: string, value: LiquidationRulePeriod) => {
    setRules((prev) => prev.map((r) => (r.id === ruleId ? { ...r, periodDays: value } : r)));
  }, []);

  const handleRulePromotionChange = useCallback((ruleId: string, value: number) => {
    setRules((prev) => prev.map((r) => (r.id === ruleId ? { ...r, promotionPercent: value } : r)));
  }, []);

  const handleRuleRedLabelChange = useCallback((ruleId: string, enabled: boolean) => {
    setRules((prev) => prev.map((r) => (r.id === ruleId ? { ...r, redLabelEnabled: enabled } : r)));
  }, []);
  ...
  const breadcrumbItems = [
    { label: "Estrategia de baja rotación", href: "/inventario/liquidaciones" },
      ...
      <MainLayout>
        <Box sx={{ py: 3 }}>
          ...
          <Typography
            ...
            sx={{ mt: 2, display: "inline-block", color: "primary.main", cursor: "pointer", textDecoration: "underline" }}
          >
              ...
              <RulesList>
                {rules.length === 0 ? (
                  <Typography color="text.secondary">
                    No hay reglas configuradas. Agrega una regla para aplicar promociones por bajo movimiento.
                  </Typography>
                ) : (
                  rules.map((rule) => (
                    <LiquidationRuleCard
