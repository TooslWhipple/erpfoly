...
export default function ConfirmarTraspasoPage() {
  ...
  const [status, setStatus] = useState<
    "loading" | "idle" | "submitting" | "empty" | "error"
  >("loading");
  ...
  const handleBack = () => {
    ...
    router.push({
      ...
      query:
        orderData?.supplierId && orderData?.supplierName
          ? {
    ...
    try {
      ...
      if (orderData.orderType === "external") {
        ...
        if (!orderData.originBranchId) {
          showError(
            "Falta la sucursal de origen. Vuelve al formulario e intenta de nuevo.",
          );
          setStatus("error");
          return;
        }
      ...
      const payload = {
        ...
        branch_id: branchId,
        origin_branch_id:
          orderData.orderType === "internal" && orderData.originBranchId
            ? Number(orderData.originBranchId)
            : undefined,
        folio: `PED-${Date.now()}`,
        order_date: dayjs().format("YYYY-MM-DD"),
        supplier_id:
          orderData.orderType === "external" && orderData.supplierId
      ...
      showSuccess(
        result.data?.message ?? "El pedido fue confirmado correctamente.",
      );
      const returnUrl =
  ...
  const breadcrumbs: BreadcrumbItem[] = orderData
    ? [
        {
          label: "Traspasos",
          href: orderData.orderType === "internal" ? "/traspasos" : "/pedidos",
        },
        {
          label:
            orderData.orderType === "internal"
              ? orderData.branchName
                ? `Sucursal ${orderData.branchName}`
                : "Sucursales"
              : orderData.supplierName
                ? `Proveedor ${orderData.supplierName}`
                : "Proveedores",
          href:
            orderData.orderType === "internal"
              ? orderData.branchId
                ? `/traspasos?branch=${orderData.branchId}`
                : "/traspasos"
              : orderData.supplierId
                ? `/pedidos?supplier=${orderData.supplierId}`
                : "/pedidos",
        },
        {
      ...
      <>
        ...
        >
          <Typography variant="body1" color="text.secondary">
            No hay artículos para confirmar. Agrega artículos en el nuevo
            traspaso.
          </Typography>
          <Button
            variant="contained"
          ...
          <Stack spacing={2}>
            {items.map((item) => (
              <ConfirmOrderItemCard
                ...
                onQuantityChange={
                  isInternalOrder ? undefined : handleQuantityChange
                }
              />
            ))}
          </Stack>
              ...
              {isInternalOrder ? (
                <Stack direction="row" justifyContent="space-between">
                  ...
                  <Typography variant="h6" fontWeight={700}>
                    {totalQuantity}{" "}
                    {totalQuantity > 1 ? "Artículos" : "Artículo"}
                  </Typography>
                </Stack>
              ) : (
                <>
