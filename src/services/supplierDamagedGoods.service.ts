...
export async function fetchSupplierDamagedGoods(
  ...
  try {
    const response = unwrapOrThrow(await get(`/suppliers/${supplierId}/dashboard/damaged-goods`));
...
export async function registerDamagedGoodsExit(
  ...
  try {
    const response = unwrapOrThrow(await post(`/suppliers/${payload.supplierId}/dashboard/damaged-goods/exit`, {
